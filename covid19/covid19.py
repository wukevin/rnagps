...
RNAGPS = utils.load_sklearn_model(RNAGPS_PATH, strict=True, disable_multiprocessing=True)
RNAGRU = utils.load_pytorch_model(recurrent.GRULocalizationClassifier, RNAGRU_PATH, device=DEVICE)
RNAGRU.eval()

GENBANK_QUERY_CACHE_FILE = os.path.join(os.path.dirname(__file__), "genbank_query_cache.json")
GENBANK_FETCH_CACHE_FILE = os.path.join(os.path.dirname(__file__), "genbank_fetch_cache")  # OS adds extension
...
PREFIXES_TO_ABLATIONS.update(
    {("gru_" + k) : v for k, v in PREFIXES_TO_ABLATIONS.items()}
)
...
def normalize_feature_labels(n:str, suffixes:List[str]=PROTEIN_SUFFIXES, name_map=PROTEIN_NAME_MAP):
    """
...
def featurize(transcript_parts:List[str], ablation_parts:List[bool]=ABLATION) -> np.ndarray:
    """Generate feature vector for the given transcript parts"""
    assert len(transcript_parts) == len(ablation_parts) == 3
    transcript_parts = ["" if ablate else part.replace("X", "N", len(part)) for part, ablate in zip(transcript_parts, ablation_parts)]
    encodings = [kmer.sequence_to_kmer_freqs(part, kmer_size=s) for s in [3, 4, 5] for part in transcript_parts]
    seq_encoded = np.hstack(encodings)
    return seq_encoded.reshape(1, -1)  # Single example

def featurize_gru(transcript_parts:List[str], ablation_parts:List[bool]) -> np.ndarray:
...
def query_genbank(query:str=COVID19_QUERY_SEQUENCE, retmax:int=9999, use_file_cache:bool=True, file_cache:str=GENBANK_QUERY_CACHE_FILE) -> List[str]:
    """
    ...
    if query not in cached_results:
        ...
        handle = Entrez.esearch(db='nucleotide', term=query, idtype='acc', retmax=retmax)
...
def fetch_genbank(identifier:str, file_cache:str=GENBANK_FETCH_CACHE_FILE) -> SeqRecord.SeqRecord:
    """
    ...
    if file_cache:
        with shelve.open(file_cache, writeback=False) as cache:  # Writeback need not be true to store new entries
            if identifier in cache:
                seq_record = cache[identifier]
            else:
                with Entrez.efetch(db='nucleotide', rettype='gb', retmode='text', id=identifier) as handle:
...
def genbank_to_feature_dict(gb_record:SeqRecord.SeqRecord, infer_utr:bool=True) -> dict:
    ...
    if not three_utr and infer_utr:
        ...
        three_utr = SeqFeature(FeatureLocation(start=retval['CDS'][-1].location.end, end=len(gb_record.seq), strand=+1))
        retval["3'UTR"].append(three_utr)
    return retval

def seq_feature_to_sequence(seq_feature:SeqFeature, genome:str, three_stop:SeqFeature=None, ablation:list=[]) -> str:
    """
    ...
    def handle_feature_location(x:FeatureLocation, three_loc:FeatureLocation=None):
        ...
        for motif in ablation:
            ...
            elif isinstance(motif, tuple) and tuple(map(type, motif)) == (SeqFeature, str):
                s = s.replace(motif[1], "X" * len(motif[1]), len(motif[1]))  # Simple version
            else:
                raise TypeError(f"Unrecognized type for ablation {type(motif)}")

        return s

    def handle_compound_location(x:CompoundLocation, three_loc:FeatureLocation=None):
...
def get_feature_labels(record:SeqRecord.SeqRecord, suffixes:List[str]=PROTEIN_SUFFIXES, name_map:dict=PROTEIN_NAME_MAP) -> List[str]:
    ...
    if not retval:
        try:
            ...
            pass
    retval = [normalize_feature_labels(l, suffixes=suffixes, name_map=name_map) for l in retval]
...
def gru_predict(featurization:np.ndarray) -> np.ndarray:
    ...
    preds = torch.sigmoid(RNAGRU(featurization)).detach().cpu().numpy()  # model outputs logits!
    return preds

def pred_feature_dict(record:SeqRecord.SeqRecord, fa_file:str="", write_trunc_seq:bool=False, ablation_motifs:Dict[str, list]=collections.defaultdict(list), use_gru:bool=False, model=RNAGPS, name_map:dict=PROTEIN_NAME_MAP, rc:bool=False, shuffle:bool=False, ablation_pos:Dict[str, List[bool]]={}) -> pd.DataFrame:
    """
    ...
    five_utr_seq = seq_feature_to_sequence(five_utr[0], genome, three_stop=None, ablation=ablation_motifs["5'UTR"] + ablation_motifs['all'])
    three_utr_seq = seq_feature_to_sequence(three_utr[0], genome, three_stop=None, ablation=ablation_motifs["3'UTR"] + ablation_motifs['all'])
    ...
    for gene, cds in zip(gene_names, ft_dict['CDS']):
        ...
        cds_seq = seq_feature_to_sequence(cds, genome, three_stop=three_utr[0], ablation=ablation_motifs["CDS"] + ablation_motifs['all'])
        cds_seq_trunc = seq_feature_to_sequence(cds, genome, three_stop=None, ablation=ablation_motifs['CDS'] + ablation_motifs['all'])  # version that doesn't run through
        ablation_pos[gene.lower()] = [base == "X" for base in cds_seq_trunc]  # We do this on truncated version
        ...
        if use_gru:
            if rc:
                featurization = featurize_gru([rev_comp(three_utr_seq), rev_comp(cds_seq), rev_comp(five_utr_seq)], ablation_parts=ABLATION)
            else:
                featurization = featurize_gru([five_utr_seq, cds_seq, three_utr_seq], ablation_parts=ABLATION)
            preds = gru_predict(featurization)
        else:
            if rc:
                featurization = featurize((rev_comp(three_utr_seq), rev_comp(cds_seq), rev_comp(five_utr_seq)), ablation_parts=ABLATION)
            else:
                featurization = featurize((five_utr_seq, cds_seq, three_utr_seq), ablation_parts=ABLATION)
            preds = list_preds_to_array_preds(model.predict_proba(featurization)).squeeze()
    ...
    pred_localization = pd.DataFrame(
        ...
        index=[data_loader.LOCALIZATION_FULL_NAME_DICT[l] for l in data_loader.LOCALIZATIONS],
    ).T
...
def pred_full_genome(record:SeqRecord.SeqRecord, use_gru:bool=False) -> pd.Series:
    ...
    """
    ft_dict = genbank_to_feature_dict(record)
    assert len(ft_dict['gene']) == len(ft_dict['CDS']), f"Got differing lengths of genes and CDS in {record}"
    assert ft_dict['CDS']
    five_utr = ft_dict["5'UTR"]
    three_utr = ft_dict["3'UTR"]
    genome = str(record.seq)
    ...
    if use_gru:
        featurization = featurize_gru([five_utr_seq, full_cds, three_utr_seq], ablation_parts=ABLATION)
        preds = gru_predict(featurization)
    else:
        featuriation = featurize((five_utr_seq, full_cds, three_utr_seq), ablation_parts=ABLATION)
...
def mean_sd_missing_vals(preds:list, min_count:int=5, verbose:bool=False) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def make_aggregate_predictions(genbank_query:str=COVID19_QUERY_SEQUENCE, ablation:List[bool]=[False, False, False], ablation_motifs:Dict[str,list]=collections.defaultdict(list), use_gru:bool=False, rc:bool=False, shuffle:bool=False, prefix:str="covid19_localization_full"):
    """
    ...
    if os.path.isfile(mean_file) and os.path.isfile(sd_file) and os.path.isfile(gmean_file) and os.path.isfile(gsd_file):
        logging.warning(f"Results appear to be complete at {prefix} - skipping (delete folder to rerun)")
        return
    ...
    for record in tqdm.tqdm(genbank_records, total=len(identifiers)):
        try:
            ...
            preds = pred_feature_dict(record, ablation_motifs=ablation_motifs, fa_file=fa_file, write_trunc_seq=True, use_gru=use_gru, rc=rc, shuffle=shuffle, ablation_pos=this_ablation_record)
...
def make_regional_predictions(prefix:str="covid19_localization_by_country"):
    ...
    genbank_full_genome_preds_by_country = collections.defaultdict(list)  # List of Series
    for record in tqdm.tqdm(genbank_records, total=len(identifiers)):
        try:
            preds = pred_feature_dict(record)
            country = get_country(record)
            genbank_preds_by_country[country][record.name] = preds
            genbank_full_genome_preds_by_country[country].append(pred_full_genome(record))
        except (AssertionError, ValueError, KeyError) as e:
            continue
    for country in genbank_full_genome_preds_by_country.keys():
        if not country or len(genbank_full_genome_preds_by_country[country]) < 5:  # Skip unknown countries or those with too few data
            continue
        preds_mean, preds_sd = mean_sd_missing_vals(genbank_preds_by_country[country].values())
        preds_mean.to_csv(f"{prefix}/{country}_mean.csv")
        preds_sd.to_csv(f"{prefix}/{country}_sd.csv")

        genome_preds_stacked = np.vstack([p.values for p in genbank_full_genome_preds_by_country[country]])
        genome_preds_mean = pd.Series(
            np.mean(genome_preds_stacked, axis=0),
            index=[data_loader.LOCALIZATION_FULL_NAME_DICT[l] for l in data_loader.LOCALIZATIONS]
        )
