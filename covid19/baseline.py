...
MODEL_PATH = os.path.join(
    os.path.dirname(SRC_DIR), "models/rf_8way_fold5.0.21.3.skmodel",
)
...
HUMAN_BASELINE_FILE_MITO_DROP = "baselines/baseline_human_apex_mito_drop.csv"
...
HUMAN_BASELINE_LABELS_FILE_MITO_DROP = (
    "baselines/baseline_human_apex_labels_mito_drop.csv"
)
...
class Baseline(object):
    ...
    def __init__(
        self,
        values_fname: str = HUMAN_BASELINE_FILE,
        labels_fname: str = "",
        invert_labels: bool = False,
        score_func=rank,
    ):
        """
        Invert labels is useful for calculating rank sum tests
        """
        logging.info(f"Loading human APEX baseline from {values_fname}")
        self.values = np.loadtxt(values_fname)
        assert len(self.values.shape) == 2
        if labels_fname:
            logging.info(f"Loading human APEX baseline labels from {labels_fname}")
        self.labels = np.loadtxt(labels_fname).astype(bool) if labels_fname else None
        if self.labels is not None:
            assert self.labels.shape == self.values.shape, f"Got mismatched shapes"
            if invert_labels:
                logging.info("Inverting labels - negatives become positive")
                self.labels = np.invert(self.labels)
    ...
    def score_vec(self, values: np.ndarray):
        ...
        assert (
            values.size == self.values.shape[1]
        ), f"Got differing sizes: {values.size} {self.values.shape[1]}"
    ...
    def best_match(
        ...
        if hist_file:
            ...
            ax.axvline(
                d[retval[-1]], linestyle="--", label=f"Cutoff at {d[retval[-1]]:.4f}"
            )
            ax.set(
...
class CoronavirusBaseline(object):
    ...
    def __init__(self, values_fname: str = COV_BASELINE_FILE, score_func=rank):
        with open(values_fname, "rb") as source:
            self.values = pickle.load(
                source
            )  # Dictionary of protein names to baseline preds
...
class RankTestBaseline(object):
    """
    Uses a rank test against baseline
    """
    ...
    def __init__(
        ...
        score_func=lambda x, y: stats.mannwhitneyu(x, y, alternative="greater"),
    ):
        extension = os.path.splitext(fname)[1]
        if extension == ".csv":
            self.values = np.loadtxt(fname, dtype=float)
        elif extension == ".pkl":
            with open(fname, "rb") as source:
                self.values = pickle.load(source)
        else:
            raise ValueError(f"Unrecognized file extension: {extension}")

        self.labels = np.loadtxt(fname_labels).astype(bool) if fname_labels else None
        if self.labels is not None:
            # assert self.labels.shape == self.values.shape
            if invert_labels:
                logging.info("Inverting labels to compare against negatives")
                self.labels = np.invert(self.labels)

        self.score_func = score_func

    def score_single(self, *args):
        raise NotImplementedError

    def score_vec(self, *args):
        raise NotImplementedError

    def score_mat(self, values: pd.DataFrame) -> np.ndarray:
        """
        Take a matrix of n_obs x n_localizations and test each column / localization
        against the corresponding column/localization
        Return a vector of 8 p-values without correction
        """
        assert self.values.shape[1] == values.shape[1]
        retval = []
        for i in range(self.values.shape[1]):
            if self.labels is not None:
                control = self.values[self.labels[:, i].flatten() == 1, i]
            else:
                control = self.values[:, i]
            experiment = values.iloc[:, i]
            _stat, pval = self.score_func(experiment, control)
            # print(np.mean(experiment) - np.mean(control), _stat, pval)
            retval.append(pval)
        assert len(retval) == self.values.shape[1]
...
def load_apex_test_dataset(fold: int = 5):
    ...
    test_dataset = data_loader.LocalizationClassificationKmers(
        split="test", k_fold=fold
    )
    return test_dataset


@functools.lru_cache(maxsize=1)
def load_apex_full_dataset():
    """Loads in the full dataset, including train/valid/test"""
    return data_loader.LocalizationClassificationKmers(split="all", k_fold=5)


def get_human_apex_baseline(
    values_fname: str, labels_fname: str = "", fold: int = 5, model=RNAGPS
):
    """
...
def get_cov_baseline(
    values_fname: str,
    use_gru: bool = False,
    model=RNAGPS,
    metadata_dict: dict = collections.defaultdict(list),
    ...
    for record in tqdm.tqdm(genbank_records, total=len(identifiers), disable=False):
        try:
            preds = covid19.pred_feature_dict(
                record, use_gru=use_gru, model=model
            )  # Produces a df (genes x localization)
...
def generate_baselines():
    ...
    get_human_apex_baseline(
        HUMAN_BASELINE_FILE_COMPOUND,
        HUMAN_BASELINE_LABELS_FILE,
        model=NUC_CYTO_COMPOUND_MODEL,
    )
    get_cov_baseline(
        COV_BASELINE_FILE_COMPOUND, use_gru=False, model=NUC_CYTO_COMPOUND_MODEL
    )
...
def find_similar_apex_sequences(
    identifier: str, top_n: int = 5, max_dist: float = 0.075
    ...
    preds_file = os.path.join(
        "covid19_localization_full", "single_preds", f"{identifier}.csv"
    )
...
def load_viral_parts(identifier: str) -> Dict[str, List[str]]:
    fasta_file = os.path.join(
        ...
        "covid19_localization_full",
        "sequences.fa",
    )
    fasta_dict = {
        tuple(k.split("|")): v
        for k, v in read_file_as_sequence_dict(fasta_file).items()
    }
...
def create_meme_fasta():
    ...
    for i in tqdm.tqdm(identifiers):
        ...
        make_fasta_for_meme(
            i, f"msa_fasta/{i}", include_5=False, include_cds=True, include_3=False
        )
